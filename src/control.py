...
from sim_types import ControlAction, ControlStrategy, GeoLocation, ProfilerState, RealProfilerState
from particle_mover import propagate_covariance_through_cycle, quickly_estimate_next_surface_position
from data_loader import build_velocity_interpolator
from noise import Q_DEFAULT
...
class MPCwithFavourable(ControlStrategy):
    ...
    def predict_next_surface_position(
        self,
        profiler_state: RealProfilerState,
        forecast: xr.Dataset,
        action: ControlAction,
    ) -> RealProfilerState:
        """Estimate where the float will surface after executing *action*."""
    ...
    def flow_term(
        ...
        predicted_state: RealProfilerState,
    ) -> float:
    ...
    def get_action(self, profiler_state: RealProfilerState, forecast: xr.Dataset, **kwargs) -> ControlAction:
        """Evaluate all candidate actions and return the one with the lowest cost.

        Parameters
        ----------
        profiler_state:
            Current real float state (position and time).
        forecast:
...
class MPCwithKalman(MPCwithFavourable):
    """MPCwithFavourable extended with an EKF uncertainty penalty.

    Adds ``uncertainty_weight * trace(P[:2,:2])`` to each candidate's cost,
    where P is the propagated position covariance at the end of the predicted
    cycle. This penalises actions that allow large position uncertainty to
    accumulate, favouring dives that keep the float's estimated position
    well-constrained.

    Parameters
    ----------
    default_action:
        Reference action (speeds, transmission duration).
    target_location:
        ``[lat, lon]`` of the target in decimal degrees.
    debug:
        Enable live debug plot.
    uncertainty_weight:
        Weight on trace(P[:2,:2]) in the cost function. Default 0.001.
        Units: cost per m². Tune relative to distance_weight (cost per km/hr).
    Q:
        Process noise covariance (4×4). Defaults to Q_DEFAULT from noise.py.
    """
    ...
    def __init__(
        ...
        target_location: list[float],
        debug: bool = False,
        uncertainty_weight: float = 0.001,
        Q: np.ndarray | None = None,
    ) -> None:
        super().__init__(default_action, target_location, debug)
        self.uncertainty_weight = uncertainty_weight
        self.Q = Q if Q is not None else Q_DEFAULT
        self.name = "mpc_with_kalman"
    ...
    def get_action(
        ...
        **kwargs,
        ...
        """Evaluate all candidate actions with EKF uncertainty penalty.

        If *est_state* is None the uncertainty term is skipped and this
        method behaves identically to :class:`MPCwithFavourable`.
        """
        if est_state is None:
            return super().get_action(profiler_state, forecast, **kwargs)

        forecast_interpolators = build_velocity_interpolator(forecast)
        next_state = self.predict_next_surface_position(profiler_state, forecast, self.last_action)

        best_cost, best_cfg, best_state = math.inf, self._action_configs[0], None
        all_candidates: list[tuple[GeoLocation, float, str]] = []

        for cfg in self._action_configs:
            action     = self._build_action(cfg)
            predicted  = self.predict_next_surface_position(next_state, forecast, action)
            d_term     = self.distance_term(predicted.location, next_state.location, action)
            f_term     = self.flow_term(forecast_interpolators, predicted)
            P_final    = propagate_covariance_through_cycle(est_state, action, forecast, self.Q)
            u_term     = self.uncertainty_weight * float(np.trace(P_final[:2, :2]))
            cost       = self.distance_weight * d_term + self.flow_weight * f_term + u_term

            depth_str = f"{cfg['target_depth']:.0f} m" if cfg["target_depth"] is not None else "bottom"
            logger.debug(
                "  %-14s %-7s %5.0fh | d=%+.4f  f=%+.4f  u=%+.4f | cost=%+.4f",
                cfg["park_mode"], depth_str, cfg["cycle_hours"],
                d_term, f_term, u_term, cost,
            )

            all_candidates.append((predicted.location, cost, self._action_color(cfg)))
            if cost < best_cost:
                best_cost, best_cfg, best_state = cost, cfg, predicted

        chosen = self._build_action(best_cfg)
        self.last_action = chosen
        self._cycle += 1

        depth_str = f"{best_cfg['target_depth']:.0f} m" if best_cfg["target_depth"] is not None else "bottom"
        logger.info(
            "Cycle %d | CHOSEN: %s %s %gh | total_cost=%+.4f",
            self._cycle, best_cfg["park_mode"], depth_str, best_cfg["cycle_hours"], best_cost,
        )
